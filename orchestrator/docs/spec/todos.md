...
> **Scope note (amended 2026-07-12, PhaseRunner collapse):** this is a historical working log; entries are not rewritten in place. Many entries below (T-01–T-03, T-09, T-11–T-19, T-25) tracked design and implementation of `PhaseRunner` and its supporting components, since deleted from the orchestrator and moved to `factory/` — their resolutions stand as a record of that work, not a description of current orchestrator code. See the repo-root `docs/spec/prd.md` and `docs/adr/0002-factory-owns-flow-control-orchestrator-is-a-trigger.md`.

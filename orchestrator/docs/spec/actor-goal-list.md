...
| ID    | Actor    | Goal                                                                                                          | Level       |
| ----- | -------- | ------------------------------------------------------------------------------------------------------------- | ----------- |
| AG-01 | Operator | Run a single agent step in isolation and get its artifact                                                     | User Goal   |
| AG-02 | Operator | Drive one phase's author↔reviewer loop to a clean review                                                      | User Goal   |
| AG-03 | Operator | Drive the project through all phases in dependency order, running one phase at a time and approving each gate | User Goal   |
...
| AG-06 | Operator | Resume an interrupted run without losing or corrupting state                                                  | User Goal   |
| SF-01 | (system) | Isolate an agent session in a fresh CLI subprocess                                                            | Subfunction |
| SF-02 | (system) | Run the deterministic gate (commit → pre-commit hooks)                                                        | Subfunction |
| SF-03 | (system) | Record a finding to the local store                                                                           | Subfunction |
| SF-04 | (system) | Evaluate the loop condition (open findings + gate result)                                                     | Subfunction |
...
| AG-11 | Operator | Run a single skill from an agent in isolation                                                     | User Goal   |
...
| SF-08 | (system) | Resolve the default model from an agent's declared tier and the active adapter's model dictionary | Subfunction |

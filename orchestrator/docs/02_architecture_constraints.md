...
| TC-3 | **GitHub Copilot CLI** is installed and authenticated for the MVP.                                                                                                          | C3                     | Copilot is the first concrete CLI adapter; the adapter and its auth handling live in `factory/`, and the orchestrator records the chosen adapter as an operator default.                |
...
| TC-5 | **Reuse `agents/`, `skills/`, `scripts/` as-is**, resolved from the package-relative path, exposed in target projects via symlinks (ADR-0010).                              | C5, NG1                | `factory/` *drives* these assets; the orchestrator reads agent front-matter (`tier`, `interactive`, `outputs`) for registry and display, and re-implements no agent or skill behaviour. |
...
| OC-2 | **Human judgement is reserved for phase gates.**                                                                                         | G5, FR-G      | `factory/` runs autonomously *within* a phase and stops *at* phase boundaries, where the orchestrator records the human's approval. |

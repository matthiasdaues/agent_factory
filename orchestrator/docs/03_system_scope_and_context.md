...
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Operator** (person)                 | Drives a project through the chain, one phase at a time.                                                                            | Runs `run-step` / `run-phase` / `status` / `resume` / `approve` as direct-mode subcommands, or drives the same functions through the interactive TUI menu (bare `orchestrate`); answers the approval prompt at phase gates. |
| **AI CLI** (external)                 | Copilot / Claude / Gemini, the engine that runs an agent.                                                                           | The orchestrator invokes it non-interactively in a fresh subprocess; the CLI writes phase artifacts to the working tree.                                                                                                    |
| **Git + pre-commit** (external)       | The version-control host and its hook set — the gate bus.                                                                           | The orchestrator commits staged artifacts on the run branch; `pre-commit` runs the hooks (starting with `spec-lint`) as the deterministic gate.                                                                             |
| **Tooling assets** (package-relative) | Agent definitions, skills, and lint scripts, resolved from the package path and exposed in target projects via symlinks (ADR-0010). | The orchestrator reads agent front-matter (author/reviewer, `outputs`), composes prompts from agent definitions, and runs lint scripts as gate hooks.                                                                       |
...
| ------------------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| AI CLI                    | out       | `subprocess` — fresh process, composed prompt, working directory, timeout; returns exit status + captured output                                     | `CLIAdapter`                     |
...
- **Deterministic findings** enter through the gate (`spec-lint --format json`); **semantic findings** enter through the reviewer agent (`inspect-spec`). Both are mapped to the same `Finding` DTO and land in the same store in the same shape (FR-E3).
- The **only** state the orchestrator owns is the findings store and `.orchestrator/`. Everything else (the spec, the arc42 docs, the code) is produced by the agents into the git working tree.

## 3.3 Scope boundary

**Inside** the system: sequencing agents, enforcing the loop policy, running the gate, owning finding IDs and lifecycle, persisting run state, and pausing for approval.

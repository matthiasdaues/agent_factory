...
The MVP runs entirely on the operator's machine (NFR-5). There is no server, no container, no network service — the orchestrator is a Python process that reads and writes the local git repository and its run state; `factory/` spawns the CLI subprocesses.
...
- **Agent subprocess** — each agent invocation is a fresh OS process (ADR-0002), spawned by `factory/`, not the orchestrator; the agent commits its work; pre-commit hooks fire inside the subprocess (ADR-0013).
...
cd my-project
orchestrate status            # observe the run
```

`orchestrate init` creates the project directory, runs `git init`, creates symlinks (`agents/`, `skills/`, `scripts/` → package-relative paths), scaffolds `docs/spec/`, `docs/adr/`, `docs/reviews/`, `backlog/`, copies `model-matrix.conf`, creates the CLI instruction file, and updates `.gitignore`. See [ADR-0010](adr/0010-separate-tooling-from-project-directory.md) for the full design. Phase execution runs through `factory/` (see the repo-root [`docs/spec/prd.md`](../../docs/spec/prd.md)); `orchestrate status` and `orchestrate approve` observe and gate it.

Once Tiers A–C are satisfied, phase execution via `factory/` is ready, and `orchestrate status`/`approve` observe and gate it. A missing Tier-A item does not corrupt state: an unauthenticated adapter or a missing/erroring gate hook halts the run with a surfaced reason (BR-015, BR-018), rather than proceeding.
...
- The CLI adapter in `factory/` already hides how the CLI is invoked and authenticated, so a CI environment differs only in the concrete adapter's auth handling.

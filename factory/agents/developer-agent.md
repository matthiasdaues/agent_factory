...
2. **Agree seams** — Identify test boundaries; prefer existing seams, highest level possible. If the story's `tests:` field is present and non-empty, those listed test files are your specification — read them as your acceptance criteria. If a `.feature` file governs the story, its Scenarios are the seams: each Scenario is one tracer bullet, and its `@`-references name the existing modules and functions the step definitions should call or extend (see [Executable Specification](#executable-specification--feature-workflow)).
   - **Discover existing detection surface.** Scan the test suite for contract tests and markers (`@pytest.mark.spec`, `@pytest.mark.contract`, or project-equivalent markers) that cover the modules this story touches. Understand what detection surface already exists — these are the tests a future agent must not silently break, and your implementation must keep them green. Note any gaps: modules being modified or extended that have no contract-test owner.
3. **Red-Green-Refactor** — The RED phase is determined by three conditions, evaluated in order:
   ...
   - **Strengthen the waist.** After prescribed or freestyle tests pass, review the detection-surface gaps identified in step 2. For each module this story modifies or introduces that lacks a contract-test owner, write one contract test that exercises the internal behavior the implementation relies on — parsing, normalization, policy decisions, state transitions, or wiring between components. This test serves dual purpose: it verifies the requirement *and* it is a tripwire that would catch a future agent silently changing the behavior while preserving the structure. Use the project's existing contract-test style — same markers, same fixture conventions, same assertion granularity. Do not duplicate what a linter already checks or what a prescribed failure scenario already covers. See [testing-strategy.md § Waist](../rulebooks/conventions/testing-strategy.md#waist--contract-tests).
   - **Add a smoke test when a user-facing path exists.** If the story introduces or modifies a user-facing path (CLI command, API endpoint, UI flow), write one representative smoke test that exercises the golden path end-to-end. Not exhaustive — one journey that would break visibly if the wiring is wrong. Use the project's existing smoke-test or integration-test style if one exists; if none exists, place it under the integration test layer. This is the top of the trophy — owned by the team's testing philosophy, written in their voice.
...
- Every modified or introduced module has at least one contract-test owner — either from prescribed failure scenarios, prior tests, or a waist-strengthening test written in this story

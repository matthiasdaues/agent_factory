...
def _git_env(tmp_path: Path) -> dict[str, str]:
    ...
    env.update(GIT_CONFIG_NOSYSTEM="1", HOME=str(tmp_path), PYTHONDONTWRITEBYTECODE="1")
    return env
...
def test_smoke_failure_escalation_and_redispatch_to_completion(
    ...
    prepare = _run_dispatch_with_ledger(
        ledger_path, "prepare-wave", "1", cwd=repo, tmp_path=tmp_path
    )
    ...
    merge = _run_dispatch_with_ledger(
        ledger_path, "merge-story", "ST-001", cwd=repo, tmp_path=tmp_path
    )
    assert merge.returncode == 0, merge.stderr
    assert "status: done" in (repo / "backlog" / "ST-001.md").read_text()

    close = _run_dispatch_with_ledger(
        ledger_path, "close-wave", "1", cwd=repo, tmp_path=tmp_path
    )

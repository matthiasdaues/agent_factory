...
class TestBranchRequiresWorktree:
    ...
    @pytest.mark.parametrize(
        "command",
        [
            "git worktree add -b feat/x .current-work/feat-x main",
            "git branch --list",
    ...
    def test_worktree_outside_current_work_is_blocked(self):
        result = _run("git worktree add -b feat/x /tmp/feat-x main")
        assert result.returncode == 2
        assert ".current-work/" in result.stderr

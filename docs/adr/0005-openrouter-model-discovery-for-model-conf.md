...
`config/model.conf` is an user-curated, per-CLI tier router: a tier (`economy`, `standard`, `strong`) either has a concrete model configured for the active CLI or it does not, and `on_missing = halt` stops a run rather than guess. It currently carries three CLI tier blocks: `copilot.*` and `codex.*` resolved directly by their native runtimes, and `pi.*` routed through OpenRouter per this decision (`pi.economy`, `pi.standard`, `pi.strong`).

Pi is being pointed at OpenRouter, which exposes hundreds of models across many providers through one endpoint, `https://openrouter.ai/api/v1/models`, each row carrying an ID, context length, and per-token pricing. Two questions follow: what concrete model does each Pi tier resolve to, and how does an user discover and keep those choices current as OpenRouter's catalog churns?
...
- **Hand-maintain the rows with no tooling.** Workable, but leaves the user reading a web catalog of hundreds of models by eye and gives no way to detect when a curated ID silently disappears. The discovery script is cheap and removes both problems; rejected as false economy.
...
- You get real help curating and validating tier rows against a large, churning catalog, without that machinery ever touching a run.
...
- Tier "suggestions" encode a price/capability heuristic that will not match every user's judgment — deliberately advisory, edited by hand, never authoritative.

...
`AF_SESSION_LOG` was declared in `factory/scripts/_session_log.py`, documented in `factory/docs/factory-guide.md` and `docs/proposals/session-log-addendum.md`, but never provisioned by any mechanism until the orchestrator's `_provision_session_log()` was added late in the session.

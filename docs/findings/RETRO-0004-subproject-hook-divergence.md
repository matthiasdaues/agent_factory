...
The orchestrator pre-commit hooks initially used `uv run --project orchestrator mdformat` and `uv run --project orchestrator ruff`, requiring a `pyproject.toml` in the orchestrator subproject. The factory hooks use `.agent-factory/factory/scripts/mdformat` (which calls `uvx` internally) and `uvx ruff`. The inconsistency caused a build failure, a false-start `pyproject.toml`, and three hook rewrites.

**Fix:** Document the rule in `.agent-factory/factory/docs/factory-guide.md` § Pre-commit hooks: subproject hooks must use `.agent-factory/factory/scripts/` wrappers and `uvx`, not `uv run --project`. When adding a subproject, copy hook entries from the factory section and change only the `files:` pattern.

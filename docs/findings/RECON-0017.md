...
severity: major
category: defect
artifact: .agent-factory/factory/config/pre-commit-config.yaml
status: resolved
...
`.agent-factory/factory/README.md` § "Test execution hooks" both document a pre-push hook that
...
- `.agent-factory/factory/config/pre-commit-config.yaml` — the canonical config
...
- This repo's own `.pre-commit-config.yaml` has a single test hook,
  ...
  `^(src/|tests?/)`. No `stages: [pre-push]` entry exists anywhere; a `git log -S pre-push -- .agent-factory/factory/config/pre-commit-config.yaml` search returns
  nothing — the hook was never added.

The only full-suite enforcement that exists is the FSM `tests_pass`
entry condition (`.agent-factory/factory/scripts/run-tests --full`) evaluated by
...
hook to `.agent-factory/factory/config/pre-commit-config.yaml` (id prefixed
...
truth, and `.agent-factory/factory/README.md` correctly restates it; the configuration is what

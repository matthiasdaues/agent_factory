...
| ------------------------------------------------------------------------------- | ---------------------------------------------------- | -------- | -------- |
| RECON-0017 — pre-push full-suite test gate is documented but never wired (open) | .agent-factory/factory/config/pre-commit-config.yaml | Defect   | Major    |
...
ADR-0003 (accepted, unsuperseded) and `.agent-factory/factory/README.md` § "Test execution
hooks" both document a pre-push hook that runs `.agent-factory/factory/scripts/run-tests --full`
...
(`.agent-factory/factory/config/pre-commit-config.yaml`) defines no `pre-push`-stage hook and
...
Confirmed against the repo: `grep run-tests .agent-factory/factory/config/pre-commit-config.yaml`
...
- Add `agent_factory_hook-run-tests-full` to the existing `- repo: local`
  block in `.agent-factory/factory/config/pre-commit-config.yaml` with `entry: .agent-factory/factory/scripts/run-tests --full`, `stages: [pre-push]`, `pass_filenames: false`, `always_run: true`.
...
- Add an `init-factory` test asserting the spliced consumer config contains
  the pre-push hook.
- Confirm `.agent-factory/factory/README.md`'s three-point test-regime description matches what

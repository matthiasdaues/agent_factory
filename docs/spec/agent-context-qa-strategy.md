...
| --------------------- | ------- | ---------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| Deterministic linter  | planned | Factory convention fallback (no linter layer declared in charter)                        | Validate YAML structure, key presence, mode field values, and reference resolution in context-lint output                                           | CX-FILE, CX-PARSE, CX-KEYS, CX-NULL, CX-MODE                |
...
| `mode` field has invalid value (not primary or index)      | Gap: no explicit scenario for invalid mode value                    | Unrecognized mode could silently pass or crash                            | Contract test    | Recommend adding a scenario for CX-MODE on invalid values                  |
...
| No scenario for CX-MODE on invalid mode value (e.g., `mode: staging`)                                                                                           | Feature spec   | low      | Consider adding a scenario to agent-context.feature for invalid mode error        |

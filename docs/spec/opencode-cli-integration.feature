...
Scenario: Installer rejects when OpenCode is not installed
  Given the opencode executable is not found on PATH
  When init-factory runs with --cli opencode
  Then init-factory stops before creating any OpenCode path
  And init-factory prints an installation instruction

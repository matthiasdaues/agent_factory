...
| Writes        | The ledger, on successful state transitions that change a story entry; `close-wave` also appends one `waves:` closeout record and commits the updated ledger                                                                                                                      |
...
All subcommands are thin wrappers around the ledger state machine. `mark-failed` accepts `--class` and `--evidence` for CLI compatibility, but Phase 1 only applies the state transition. `close-wave` succeeds only when every story in the requested wave is terminal, and it records a wave closeout entry with completed, blocked, failed, next_ready, and branch_head fields.

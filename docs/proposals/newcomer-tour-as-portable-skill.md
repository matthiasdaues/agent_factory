...
impact:
  ...
  boundaries:
    ...
    - .agent-factory/factory/skills/draft-proposal/SKILL.md
...
`init-factory` copies `.agent-factory/factory/` into the target project but never copies
...
`.agent-factory/factory/docs/factory-guide.md`, before the existing "Agents" section.
...
- Internal links rewritten to `.agent-factory/factory/`-relative paths.
...
**`newcomer-tour`** (`.agent-factory/factory/skills/newcomer-tour/SKILL.md`):

> Read the "Getting Started" section of `.agent-factory/factory/docs/factory-guide.md`
...
**`explain-concept`** (`.agent-factory/factory/skills/explain-concept/SKILL.md`):

> Look up the requested concept in `.agent-factory/factory/docs/factory-guide.md` and
> `.agent-factory/factory/INDEX.yaml`. Explain it in plain language, calibrated to the
...
The kit-manager agent definition (`.agent-factory/factory/agents/kit-manager.md`) is
...
`.agent-factory/factory/config/AGENTS.md` contains five references to chat-agent and
...
`_orientation_block()` function, which reads `.agent-factory/factory/config/AGENTS.md`.
...
> [`factory/docs/factory-guide.md` § Getting Started](../../.agent-factory/factory/docs/factory-guide.md).
...
Two files inside `.agent-factory/factory/` currently link to the old location:

- `.agent-factory/factory/README.md` — rewrite to point to
  `.agent-factory/factory/docs/factory-guide.md` (Getting Started section).
- `.agent-factory/factory/docs/factory-guide.md` — the opening line that says "start
...
- `.agent-factory/factory/agents/virgil.md` — new agent definition. Rename and expand
  chat-agent. Adopt pattern, skill-aware, thin role statement.
- `.agent-factory/factory/skills/newcomer-tour/SKILL.md` — new skill for the
  newcomer walkthrough.
- `.agent-factory/factory/skills/explain-concept/SKILL.md` — new skill for
  on-demand factory concept explanations.
- `.agent-factory/factory/skills/comic-relief/SKILL.md` — new skill for
  context-aware nerdy humor (Dilbert × XKCD × South Park triangle).
- `.agent-factory/factory/docs/factory-guide.md` — add "Getting Started" section;
  remove orchestrator and project-specific references; add seam
  between beginner and reference material.
- `.agent-factory/factory/config/AGENTS.md` — all chat-agent and kit-manager
  references updated to VIRGIL (options A, B7, D, adopt-pattern
  routing, Codex example).
- `.agent-factory/factory/skills/guided-tour/SKILL.md` — update option A description
  to reference VIRGIL; retain reorientation role.
- `.agent-factory/factory/README.md` — beginner-intro link updated to factory-guide
  Getting Started section.
- `docs/arc42/beginner-intro.md` — replaced with redirect stub.
- `.agent-factory/factory/agents/kit-manager.md` — retired; workflow absorbed into
  VIRGIL's agent definition.
- `.agent-factory/factory/agents/chat-agent.md` — retired; superseded by VIRGIL.
- `.agent-factory/factory/skills/draft-proposal/SKILL.md` — chat-agent references
  updated to VIRGIL.
- `.agent-factory/factory/skills/detect-test-regime/SKILL.md` — kit-manager references
  updated to VIRGIL.
- `.agent-factory/factory/INDEX.yaml` — register VIRGIL agent and new skills; remove
...
VIRGIL lives in `.agent-factory/factory/agents/`, which ships with the factory. Its
knowledge base is `.agent-factory/factory/docs/factory-guide.md`, also inside
`.agent-factory/factory/`. Both are available in any project after `init-factory`.
...
`.agent-factory/factory/skills/` and are symlinked into each CLI's skill directory
...
- A fresh project with only `.agent-factory/factory/` installed (no `docs/arc42/`
...
- VIRGIL explains advanced factory concepts on demand from the
  reference sections of `.agent-factory/factory/docs/factory-guide.md`.
...
- `.agent-factory/factory/docs/factory-guide.md` opens with a "Getting Started"
...
- `.agent-factory/factory/README.md` links to the factory-guide Getting Started
  section.
- `.agent-factory/factory/agents/chat-agent.md` and `.agent-factory/factory/agents/kit-manager.md`
...
3. **Simplification:** Merge beginner content into
   `.agent-factory/factory/docs/factory-guide.md` as a "Getting Started" section.
...
| PROP-05 | minor    | 08    | resolved | Arithmetic inconsistency in estimate. Resolution: corrected.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| PROP-06 | minor    | 08    | resolved | Multiplier/playbook mismatch. Resolution: changed to `documentation-update`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| PROP-07 | major    | 02    | resolved | Design section 4 claims playbook references to kit-manager exist in greenfield-development step 1 and brownfield-onboarding step 1; they do not (zero matches in either playbook). Three skills that DO reference these agents are not in scope: `.agent-factory/factory/skills/draft-proposal/SKILL.md` (lines 5, 15: "chat-agent"), `.agent-factory/factory/skills/detect-test-regime/SKILL.md` (lines 35, 81, 189: "kit-manager"), and `.agent-factory/factory/skills/guided-tour/SKILL.md` (line 30: "chat-agent" in option D description). Resolution: removed phantom playbook claims; added `draft-proposal`, `detect-test-regime`, and `INDEX.yaml` to scope and boundaries. |
...
| PROP-09 | major    | 05    | resolved | Three boundary files missing: `.agent-factory/factory/INDEX.yaml`, `.agent-factory/factory/skills/draft-proposal/SKILL.md`, `.agent-factory/factory/skills/detect-test-regime/SKILL.md`. Resolution: added all three to boundaries list and scope.                                                                                                                                                                                                                                                                                                                                                                                                                                   |

...
| Field               | Meaning                                                      |
| ------------------- | ------------------------------------------------------------ |
| `project_id`        | stable project UUID generated by `init-factory`              |
| `project_name`      | non-empty human-readable project name configured at init     |
...
- **The tokenized transcript is persisted by default** as a copy under
  ...
  prunable by session.
- **Project identity is explicit.** A fresh `init-factory` run asks for the
  project name (or accepts `--project-name` for automation), generates a stable
  UUID, and stores both values in the git-ignored `config/project.json`.
  Capture requires that configuration and copies `project_id` and
  `project_name` into every record; rerunning initialization preserves the
  identity.

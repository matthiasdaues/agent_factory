...
Accounting is driven by a closed registry keyed by CLI:

- **Claude Code:** select the latest root snapshot and add each distinct child
  run once.
- **Pi:** select the root record and add each distinct descendant once.
- **Codex:** select the latest inclusive root snapshot; child records provide
  attribution and are not added to the total.
- **GitHub Copilot CLI:** select the latest inclusive root snapshot; child
...
| A malformed or truncated line cannot silently alter totals.                                                            | Operational preflight                         | A corpus containing valid, malformed, truncated, wrong-type, and invariant-breaking lines produces all expected diagnostics; stable queries refuse to run on an invalid selected set.                                                                                                                                                                                                                                                                                                                                                 | Query exits non-zero before DuckDB accounting runs.                         |
...
03. The supported CLI set and accounting registry match exactly.

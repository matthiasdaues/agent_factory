...
| A malformed line or run hierarchy cannot silently alter totals.                                                        | Operational preflight                         | A corpus containing valid, malformed, truncated, wrong-type, invariant-breaking, missing-parent, cross-boundary-parent, self-parent, cyclic, and multiple-root evidence produces all expected diagnostics; stable queries refuse to run on an invalid selected set.                                                                                                                                                                                                                                                                   | Query exits non-zero before DuckDB accounting runs.                         |
...
04. The accounting fixture matrix proves the declared root, snapshot, child,
    ...
    supported CLI, including all five malformed-ancestry preflight codes.

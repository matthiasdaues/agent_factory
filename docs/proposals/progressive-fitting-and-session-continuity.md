...
| `test_regime_detected`    | Yes        | `docs/agent-context/workflow.yaml#testing` is non-null and non-deferred, or `testing.yaml` exists under `docs/agent-context/` or `docs/charter/` (legacy charter fallback, per `_resolve_testing_yaml`)                                                             |

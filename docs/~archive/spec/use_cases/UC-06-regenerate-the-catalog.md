...
- **Human Operator** — wants `.agent-factory/factory/INDEX.yaml` to always describe what actually exists in `.agent-factory/factory/agents/`, `.agent-factory/factory/skills/`, `.agent-factory/factory/playbooks/`, and `.agent-factory/factory/rulebooks/`, without maintaining it by hand.
...
The actor runs `.agent-factory/factory/scripts/index-lint`, either to regenerate the catalog or (with `--check`) to verify it is current.

## Preconditions

- None — `index-lint` reads `.agent-factory/factory/agents/*.md`, `.agent-factory/factory/skills/*/SKILL.md`, `.agent-factory/factory/playbooks/*.md`, and `.agent-factory/factory/rulebooks/**/*.md` (excluding templates), all of which are ordinary tracked files.

## Main Success Scenario

1. Actor runs `.agent-factory/factory/scripts/index-lint`.
...
7. `index-lint` renders the four sections (agents, skills, playbooks, rulebooks) as YAML and compares them against the current `.agent-factory/factory/INDEX.yaml`.
...
- **Success Guarantee**: after a successful run, `.agent-factory/factory/INDEX.yaml`'s content is byte-for-byte what `index-lint` would generate from the current frontmatter — running it again immediately makes no further change. Every entry carries a `tokens` field; agents and playbooks carry `total_tokens`.
- **Minimal Guarantee**: `--check` never mutates `.agent-factory/factory/INDEX.yaml`, regardless of whether the catalog is stale.

## Business Rules

- **BR-015**: `factory/INDEX.yaml` is never hand-edited; regenerating it with `index-lint` is the only way its content changes. A playbook's agent sequence is derived from its own `**Agent**: `x\`\` prose lines, the same principle this repo applies to Mermaid diagrams under [state-machine-notation.md § Canonical Format](../../../.agent-factory/factory/rulebooks/conventions/state-machine-notation.md#canonical-format) — the source of truth is authored once, and every derived view is generated from it, never hand-duplicated.
...
flowchart TD
    ...
    G -->|no| I[Write .agent-factory/factory/INDEX.yaml, exit 0]
```
...
Feature: Regenerate the catalog
  ...
  Scenario: Regeneration writes an up-to-date catalog
    ...
    Then .agent-factory/factory/INDEX.yaml is rewritten to reflect the change
    And index-lint exits 0

  Scenario: A clean re-run changes nothing
    Given .agent-factory/factory/INDEX.yaml already matches current frontmatter
    When the actor runs index-lint
  ...
  Scenario: --check detects a stale catalog without writing
    ...
    And it exits 1
    And .agent-factory/factory/INDEX.yaml is left unchanged
```

## Referenced from

- [actor-goal-list.md](../actor-goal-list.md)
- [factory/scripts/index-lint](../../../.agent-factory/factory/scripts/index-lint)

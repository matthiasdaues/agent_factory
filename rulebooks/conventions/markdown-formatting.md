...
- `scratchpad` is the one exception to "cited, not restated": it's an executable script, not agent-followed prose, so the rule is compiled directly into `scripts/scratchpad` (calls `scripts/mdformat` after every append) rather than cited in text.

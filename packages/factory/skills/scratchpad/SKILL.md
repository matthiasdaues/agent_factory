...
```bash
echo "{note text}" | .agent-factory/factory/scripts/scratchpad
```

The script handles everything: creates the file if it doesn't exist, starts a new date section if the most recent one isn't today, appends the note as a bullet, and formats the file via `.agent-factory/factory/scripts/mdformat` per [markdown-formatting.md](../../rulebooks/conventions/markdown-formatting.md). Never construct the file/heading structure by hand, and never edit `session-scratchpad.md` directly — always go through the script, so the date-sectioning and formatting stay consistent.

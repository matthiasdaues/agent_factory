...
If invoked with `--no-stage --no-commit`: leave all changes unstaged, skip the commit, and return. Do not set `status: done`; the human owns staging, the status update, and the commit.

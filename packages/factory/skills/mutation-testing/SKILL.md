...
Set up project-owned mutation testing to verify behavioral coverage, classify
survivors, and — when a QA strategy contract-owner table exists — classify
each mutant's contract ownership.

For setup guidance (tool selection, configuration, diff scoping, CI wiring),
see the [mutation testing methodology](../../rulebooks/references/mutation-testing-methodology.md).

...
Read `docs/spec/scope-map.md` and the `.feature` files under `docs/spec/`. Read `docs/CONTEXT.md` if it exists — use the project's domain vocabulary. Read `docs/agent-context.md` and follow the concerns relevant to the feature scope — the cross-cutting concerns define Epic 0 (foundational must-haves).

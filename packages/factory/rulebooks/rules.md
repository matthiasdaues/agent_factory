...
- **MUST** read the local INDEX.yaml first (`.claude/INDEX.yaml` for Claude Code, `.github/INDEX.yaml` for GitHub Copilot CLI, `.pi/INDEX.yaml` for Pi, `.codex/INDEX.yaml` for Codex). All locally available agents, skills, and playbooks are referenced there. **Codex**: resolve `path:` entries from INDEX.yaml — agents to their generated equivalents under `.codex/agents/` (e.g. `agents/virgil.md` → `.codex/agents/virgil.toml`), skills to `.agents/skills/` (e.g. `skills/grilling/SKILL.md` → `.agents/skills/grilling/SKILL.md`). If `.codex/` does not exist, see source-repo fallback below.
...
- **MUST (Codex only)**: Resolve Factory agents to their generated definitions under `.codex/agents/`. When the user directly selects an agent for interactive work, adopt that role in the current session unless the agent definition requires a separate session. Spawn a native custom agent when a playbook dispatches work, independent author/reviewer sessions are required, or work runs in parallel. **MUST NOT** perform authoring and its independent review in the same session. Use `.codex/playbooks`, `.codex/rulebooks`, and `.codex/scripts` for the Factory aliases. **Source-repo fallback:** if `.codex/` does not exist but `factory/` does, you are in the Agent Factory source repository — resolve `path:` entries directly under `factory/` (e.g. `agents/virgil.md` → `factory/agents/virgil.md`) and read the canonical Markdown as you would under Claude Code or Copilot CLI.
...
- **MUST NOT** duplicate or override a team's existing linting and formatting toolchain — factory linters cover only factory artifact shapes the team's tools do not know about.
- **MUST** admit a test that would catch a semantic change no existing linter or structural check can see, even when the contract already has a structural owner.
...
- **MUST** create every new invocation branch from `dev` (not `main`), using `feature/<proposal-title>` as the branch name. An adopted review branch keeps its existing name and ancestry and records current `HEAD` as its review root. Story branches are cut from the invocation branch.

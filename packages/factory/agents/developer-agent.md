...
2. **Agree seams** — Identify test boundaries; prefer existing seams, highest level possible. If the story's Notes for the Implementer section lists pre-existing tests, those listed test files are your specification — read them as your acceptance criteria. If a `.feature` file governs the story, its Scenarios are the seams: each Scenario is one tracer bullet, and its `@`-references name the existing modules and functions the step definitions should call or extend (see [Executable Specification](#executable-specification--feature-workflow)).
...
3. **Red-Green-Refactor** — The RED phase is determined by three conditions, evaluated in order:
   ...
   - **Pass 2 — test-design (conditional).** After GREEN, if the story is **not** `.feature`-governed, invoke the [`test-design`](../skills/test-design/SKILL.md) skill against the implemented code. The skill reads ownership assignments from the testability probe in `backlog/epics.md`, classifies owned contracts by risk class, identifies integration paths and edge cases the TDD cycle did not cover, and authors additional test files. If the story **is** `.feature`-governed, skip this step — the `.feature` file is the test design, and the contract-test gap-fill above handles internal seams.
4. **Commit** — Per [commit-conventions.md](../rulebooks/conventions/commit-conventions.md): `feat: <description> (ST-NNNN)`, set `status: done`. Additionally, write two fields into the story file's frontmatter at commit time:
   - `tests:` — list the test modules this story owns (authored during pass 1 and pass 2). Include only modules this story created or extended, not prior tests from dependency stories.
   - `test-design-pass:` — `done` if the `test-design` skill was invoked (regardless of whether it found gaps), or `skipped-feature-governed` if the story is `.feature`-governed and pass 2 was correctly skipped.
     If invoked with `--no-commit`: stage all changed files (`git add`), skip the commit, and return a summary of staged changes and passing tests. Do not set `status: done` — the human commits after review. Still write the `tests:` and `test-design-pass` fields into the story file.

...
1. **Load backlog + initialise dispatch run** — Parse all `backlog/ST-*.md`: `id`, `status`, `deps`, `tier`, `outputs`. Build the dependency graph and identify **ready stories** (`status: pending`, all `deps` done). Read the project context from `docs/agent-context/*.yaml` (falls back to `docs/charter/*.md` for legacy projects) for model selection and dispatch strategy. Before any subagent launch, call `factory/scripts/dispatch init --base <base-branch> --stories <comma-separated-story-ids>` — the script creates the invocation branch/worktree, preflights output directories, records branch root, and initialises the ledger under `.current-work/`. If resuming, recover state from the ledger instead of reconstructing from git history.
